 ...
 *      RL-ARM - CAN
 *----------------------------------------------------------------------------
 *      Name:    CAN_STM32F107.c
 *      Purpose: CAN Driver, Hardware specific module for ST STM32F105/7
 *      Rev.:    V4.70
 *----------------------------------------------------------------------------
...
CAN_ERROR CAN_hw_setup (U32 ctrl)  {
  ...
  switch (ctrl) {
    case 1: 
      #if USE_CAN_CTRL1 == 1
        /* Enable clock for CAN1, GPIOD and AFIO */
        RCC->APB1ENR |= (1 << 25);
        RCC->APB2ENR |= (1 <<  5) | (1 <<  0);
        /* CAN1 remap, use PD0, PD1 */
        AFIO->MAPR   |= 0x00006000;
        GPIOD->CRL   &= 0xFFFFFF00;
        GPIOD->CRL   |= 0x000000B8;
        /* Enable CAN1 interrupts */
        NVIC->IP[4]  |= 0x10000000;                  // set priority
        NVIC->IP[5]  |= 0x00000010;                  // set priority
        NVIC->ISER[0] = (1 << 20) | (1 << 19);       // enable interrupt
      #endif
      break;
    case 2: 
      #if USE_CAN_CTRL2 == 1
        /* Enable clock for CAN2, GPIOB and AFIO */
        RCC->APB1ENR |= (1 << 25) | (1 << 26);
        RCC->APB2ENR |= (1 <<  3) | (1 <<  0);
        /* CAN2 remap, use PB5, PB6 */
        AFIO->MAPR   |= 0x00400000;
        GPIOB->CRL   &= 0xF00FFFFF;
        GPIOB->CRL   |= 0x0B800000;
        /* Enable CAN2 interrupts */
        NVIC->IP[15] |= 0x10000000;                  // set priority
        NVIC->IP[16] |= 0x00000010;                  // set priority
        NVIC->ISER[1] = 1U << 31;                    // enable interrupt
        NVIC->ISER[2] = 1 << 0;                      // enable interrupt
      #endif
      break;
    default:
