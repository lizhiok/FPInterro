...
#define FAT_NAME_DEPTH_POOL_SZ  (FAT_NAME_DEPTH_CB_SZ * FAT_NAME_MAX_DEPTH)
...
void fs_config (void) {
 ...
 BOOL fat_getlabel (IOB *fcb, char *buf)          { return (__FALSE); }
 BOOL fat_attrib (IOB *fcb, const char *fn, U32 attrib) {
                                                    return (__FALSE); }
#else
 #if (FSJ_USE == 0)
  ...
  void *elink_alloc     (U32 i)                   { return (NULL); }
  U32   elink_cmd       (U32 i, U32 j)            { return (0);    }
  void *elink_set       (U32 i, U32 j)            { return (NULL); }
  U32   elink_prop      (void *p, U32 i)          { return (0);    }
